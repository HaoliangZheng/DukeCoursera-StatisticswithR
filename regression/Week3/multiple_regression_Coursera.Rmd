...
<ol>
<li><font color="red"> Observational study </font></li>
<li> Experiment </li>
</ol>
...
<li><font color="red"> Yes, revise wording to "Is there an association between beauty and course evaluations?" </font></li>
...
<li><font color="red"> The left skewness of the data suggests that the students are less likely to rate the professors highly. </font></li>
</ol>
```{r score-dist}
...
<li><font color="red"> Nearly normal residuals: Residuals are right skewed, but the sample size is large, so this may not be an important violation of conditions. </font></li>
...
ggplot(data = m_bty, aes(x = 1:length(.resid), y = .resid)) +
  geom_point() +
  geom_hline(yintercept = 0, linetype = "dashed") +
  xlab("Order") +
  ylab("Residuals")
...
ggplot(data = m_bty_gen, aes(x = 1:length(.resid), y = .resid)) +
  geom_point() +
  geom_hline(yintercept = 0, linetype = "dashed") +
  xlab("Order") +
  ylab("Residuals")
...
<li><font color="red"> Tenure Track, Tenured, Teaching </font></li>
...
<li><font color="red"> 0.12 points higher than minority professors, all else held constant. </font></li> 
...
```{r p-val-select}
# type your code for the Exercise here, and Knit
m_s1 <- lm(score ~ rank + ethnicity + gender + language + age + cls_perc_eval 
...
<ol>
<li> `bty_avg` </li>
<li><font color="red"> `cls_profs` </font></li> 
<li> `cls_students` </li>
<li> `rank` </li>
</ol>
